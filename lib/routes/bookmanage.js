...
router.addBook = (req, res) => {
    ...
    book.save(function (err) {
        if (err) res.json({ message: 'Book NOT Added!', errmsg: err });else res.json({ message: 'Book Successfully Added!', data: book });
    });
};
...
router.incrementamount = (req, res) => {
    ...
    } else res.send('Book NOT Found - amount updated NOT Successful!!');
};

router.incrementamount = (req, res) => {

    Book.findById(req.params.id, function (err, book) {
        if (err) res.json({ message: 'Book NOT Found!', errmsg: err });else {
            book.amount += 1;
            book.save(function (err) {
                if (err) res.json({ message: 'Book amount NOT updated!', errmsg: err });else res.json({ message: 'Book Successfully updated!', data: book });
            });
        }
    });
};
...
router.deleteBook = (req, res) => {
    ...
    Book.findOneAndRemove({ "username": req.params.username }, function (err) {
        if (err) res.json({ message: 'Book NOT DELETED!', errmsg: err });else res.json({ message: 'Book Successfully Deleted!' });
    });
};
...
router.findTotalamounts = (req, res) => {
    ...
    Book.find(function (err, books) {
        if (err) res.send(err);else res.json({ totalamounts: getTotalamounts(books) });
    });
};
