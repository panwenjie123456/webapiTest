...
router.addBooktype = (req, res) => {
    ...
    booktype.save(function (err) {
        if (err) res.json({ message: 'Booktype NOT Added!', errmsg: err });else res.json({ message: 'Booktype Successfully Added!', data: booktype });
    });
};
...
router.findByName = (req, res) => {
    ...
    var _filter = {
        $or: [{ book_name: { $regex: keyword, $options: '$i' } }]
...
router.incrementUpvotes = (req, res) => {
    ...
    } else res.send('Booktype NOT Found - UpVote NOT Successful!!');
};

router.incrementUpvotes = (req, res) => {

    Booktype.findById(req.params.id, function (err, booktype) {
        if (err) res.json({ message: 'Booktype NOT Found!', errmsg: err });else {
            booktype.upvotes += 1;
            booktype.save(function (err) {
                if (err) res.json({ message: 'Booktype NOT UpVoted!', errmsg: err });else res.json({ message: 'Booktype Successfully Upvoted!', data: booktype });
            });
        }
    });
};
...
router.deleteBooktype = (req, res) => {
    ...
    Booktype.findOneAndRemove({ "username": req.params.username }, function (err) {
        if (err) res.json({ message: 'Booktype NOT DELETED!', errmsg: err });else res.json({ message: 'Booktype Successfully Deleted!' });
    });
};
...
router.findTotalVotes = (req, res) => {
    ...
    Booktype.find(function (err, booktype) {
        if (err) res.send(err);else res.json({ totalvotes: getTotalVotes(booktype) });
    });
};
