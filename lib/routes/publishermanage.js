...
router.findOne = (req, res) => {
    ...
    Publisher.find({ "_id": req.params.id }, function (err, publisher) {
        if (err) res.json({ message: 'Publisher NOT Found!', errmsg: err });else res.send(JSON.stringify(publisher, null, 5));
    });
};
...
router.addPublisher = (req, res) => {
    ...
    publisher.save(function (err) {
        if (err) res.json({ message: 'Publisher NOT Added!', errmsg: err });else res.json({ message: 'Publisher Successfully Added!', data: publisher });
    });
};
...
router.incrementUpvotes = (req, res) => {
    ...
    } else res.send('Publisher NOT Found - UpVote NOT Successful!!');
};

router.incrementUpvotes = (req, res) => {

    Publisher.findById(req.params.id, function (err, publisher) {
        if (err) res.json({ message: 'Publisher NOT Found!', errmsg: err });else {
            publisher.upvotes += 1;
            publisher.save(function (err) {
                if (err) res.json({ message: 'Publisher NOT UpVoted!', errmsg: err });else res.json({ message: 'Publisher Successfully Upvoted!', data: publisher });
            });
        }
    });
};
...
router.deletePublisher = (req, res) => {
    ...
    Publisher.findByIdAndRemove(req.params.id, function (err) {
        if (err) res.json({ message: 'Publisher NOT DELETED!', errmsg: err });else res.json({ message: 'Publisher Successfully Deleted!' });
    });
};
...
router.findTotalVotes = (req, res) => {
    ...
    Publisher.find(function (err, publisher) {
        if (err) res.send(err);else res.json({ totalvotes: getTotalVotes(publisher) });
    });
};
